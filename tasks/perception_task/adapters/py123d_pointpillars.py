...
def lidar_to_pointpillars_points(lidar: Lidar) -> np.ndarray:
    """
    Convert a py123D LiDAR frame into the point format expected by OpenPCDet PointPillars on nuScenes.
...
def load_pointpillars_frame(
    scene: SceneAPI,
    frame_index: int = 0,
) -> np.ndarray:
    """
    Load one LiDAR frame from a py123D scene and convert it for PointPillars.
    """
    ...
    return lidar_to_pointpillars_points(lidar)