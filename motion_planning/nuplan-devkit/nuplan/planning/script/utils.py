...
def update_config_for_simulation(cfg: DictConfig) -> None:
    """Update configuration required by simulation without importing training dependencies."""
    OmegaConf.set_struct(cfg, False)

    if cfg.max_number_of_workers:
        cfg.callbacks = [
            callback
            for callback in cfg.callback.values()
            if str(callback._target_) != "nuplan.planning.simulation.callback.timing_callback.TimingCallback"
        ]
...
def set_up_common_builder(cfg: DictConfig, profiler_name: str) -> CommonBuilder:
    ...
    if cfg.enable_profiling:
        # Profiling is optional and requires PyTorch Lightning.
